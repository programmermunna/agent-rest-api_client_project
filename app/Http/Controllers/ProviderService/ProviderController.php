  ...
  public function transaction(Request $request)
  {
    ...
    if ($data->amount <= 1) {
      ...
      $win = [
        ...
        'bet_id' => $data->code,
        'round_id' => $data->roundId,
        'deskripsi' => 'Game ' . ' : '. $data->status .'Amount = '. $data->amount. ' Result = '. $data->result . ' Start balance = '.$data->startbalance. ' End balance = '. $data->endbalance. 'description = ' . $data->description,
        'game_id' => $data->gameId,
        'type' => $data->status,
        'game_info' => $data->type,
        'win' =>  $data->endbalance,
