...
class UserEventListener
{
    ...
    public function onCreated($event)
    {
        activity('user')
            ...
            ->withProperties([
                ...
                'activity' => 'Create type ' . $event->user->type ,
            ...
            ->log('with roles: :properties.roles');
    }
    ...
    public function onUpdated($event)
    {
        activity('user')
            ...
            ->withProperties([
                ...
                'target' => $event->user->name,
                'activity' => 'Update type ' . $event->user->type ,
                'roles' => $event->user->roles->count() ? $event->user->roles->pluck('name')->implode(', ') : 'None',
                'permissions' => $event->user->permissions ? $event->user->permissions->pluck('description')->implode(', ') : 'None',
            ])
            ->log('with roles: :properties.roles ');
    ...
    public function onDeleted($event)
    {
        activity('user')
            ...
            ->withProperties([
                'target' => $event->user->name,
                'activity' => 'Delete admin type ' . $event->user->type ,
            ])
    ...
    public function onStatusChanged($event)
    {
        activity('user')
            ...
            ->withProperties([
                'target' => $event->user->name,
                'activity' => ($event->status === 0 ? 'deactivated ' : 'reactivated '),
            ])
            ->log('Succesfully');
    }
